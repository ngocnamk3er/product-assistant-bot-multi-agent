...
from agents.policy_qa_agent.task_manager import PolicyQAAgentTaskManager
from agents.policy_qa_agent.agent import PolicyQAAgent
...
def main(host, port):
    ...
    agent_card = AgentCard(
        name="PolicyQAAgent",                               # Name of the agent
        description="Trả lời người dùng về các chính sách của công ty",  # Description
        url=f"http://{public_hostname}:{port}/",                       # The public URL where this agent lives
        version="1.0.0",                                    # Version number
        defaultInputModes=PolicyQAAgent.SUPPORTED_CONTENT_TYPES,  # Input types this agent supports
        defaultOutputModes=PolicyQAAgent.SUPPORTED_CONTENT_TYPES, # Output types it produces
    ...
    server = A2AServer(
        ...
        task_manager=PolicyQAAgentTaskManager(agent=PolicyQAAgent())
    )
